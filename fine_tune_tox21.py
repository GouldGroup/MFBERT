...
class tox21Dataset(Dataset):
    def __init__(self, training=True):
        classes = ['NR-AR', 'NR-AR-LBD', 'NR-AhR', 'NR-Aromatase', 'NR-ER', 
                    'NR-ER-LBD', 'NR-PPAR-gamma', 'SR-ARE', 'SR-ATAD5', 
                    'SR-HSE', 'SR-MMP', 'SR-p53']
        examples = []
        if training:
            for _, row in train.iterrows():
                examples.append((row['smiles'], np.array(row[classes], dtype=int)))
        else:
...
class MFBERTFortox21(torch.nn.Module):
    def __init__(self):
        super(MFBERTFortox21, self).__init__()
...
trainds = tox21Dataset()

model = MFBERTFortox21().cuda()
    ...
    for _ , data in tqdm(enumerate(training_loader, 0), desc='ITERATION', total=len(training_loader)):
        ...
        if _%100==0:
            ...
            if loss.item()<curminloss:
                torch.save(model, f'fine-tuned/tox21_model_best_{loss.item()}.bin')
...
torch.save(model, 'fine-tuned/tox21_model_last.bin')