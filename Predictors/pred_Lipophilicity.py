...
class MFBERTForLipo(torch.nn.Module):
    def __init__(self):
        super(MFBERTForLipo, self).__init__()
        self.l1 = None
        self.l2 = None
        self.l3 = None
...
model = torch.load('../Model/fine_tuned/Lipophilicity/pytorch_model.bin', map_location=DEVICE).to(DEVICE)


with open('../Datasets/data_splits/Lipophilicity/test.pkl', 'rb') as f:
...
with open(f'{OUTPUT_DIR}Lipo_preds_and_fps.pkl', 'wb') as f:
    pickle.dump(data,f)