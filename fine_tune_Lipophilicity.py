...
class LipoDataset(Dataset):
    def __init__(self):
        ...
        with open('Datasets/data_splits/Lipophilicity/train.pkl', 'rb') as f:
    ...
    for _ , data in tqdm(enumerate(training_loader, 0), desc='ITERATION', total=len(training_loader)):
        ...
        if _%100==0:
            ...
            if loss.item()<curminloss:
                torch.save(model, f'fine-tuned/Lipo_model_best_{loss.item()}.bin')
...
torch.save(model, 'fine-tuned/Lipo_model_last.bin')
