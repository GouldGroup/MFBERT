...
class MFBERTForHIV(torch.nn.Module):
    def __init__(self):
        super(MFBERTForHIV, self).__init__()
    ...
    for _ , data in tqdm(enumerate(training_loader, 0), desc='ITERATION', total=len(training_loader)):
        ...
        if _%100==0:
            ...
            if loss.item()<curminloss:
                torch.save(model, f'fine-tuned/HIV_model_best_{loss.item()}.bin')
...
torch.save(model, 'fine-tuned/HIV_model_last.bin')