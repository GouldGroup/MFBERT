...
model = SiameseMFBERTForBBBP().cuda()
    ...
    for _ , data in tqdm(enumerate(training_loader, 0), desc='ITERATION', total=len(training_loader)):
        ...
        if _%100==0:
            ...
            if loss.item()<curminloss:
                torch.save(model, f'fine-tuned/siamese_classification/BBBP_model_best_{loss.item()}_siamese.bin')
...
torch.save(model, 'fine-tuned/siamese_classification/BBBP_model_last_siamese.bin')
