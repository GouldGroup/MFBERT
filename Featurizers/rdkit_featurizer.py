...
OUTPUT_DIR = '../Fingerprints/rdkit_virtual_screening/'
os.makedirs(OUTPUT_DIR, exist_ok=True)

class MFBERT_rdkit_featurizer(torch.nn.Module):
    def __init__(self):
        super(MFBERT_rdkit_featurizer, self).__init__()
        self.l1 = None
...
if __name__ == '__main__':
    ...
    model = torch.load('../Model/rdkit_featurizer/pytorch_model.bin', map_location=DEVICE).to(DEVICE)
    for DATA_FILE in tqdm(os.listdir(DATA_DIR)):
        ...
        for batch in trange(0,len(data), BATCH_SIZE):
            ...
            with torch.no_grad():
                try:
                    res = model(ids,mask).detach().numpy() # numpy tensor of mean embeddings/batch
